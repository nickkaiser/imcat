...
cathead		*readcathead(void)
{
	item	*theitem;
	cathead	*thecathead;
	int	itype, done = 0, i, filetype, swapbytesoninput = 0;
	char	line[LINE_SIZE];
	comment	*thecomment;
	...
	while (!done) {
		switch (itype = gettype()) {
			...
			case TEXT_TYPE:
				...
				/* check to see if it is the byteorder header item */
				if (itype == TEXT_TYPE) {
					if (!strcmp(theitem->name, "byteorder")) {
						if (theitem->ndim == 1 && theitem->dim[0] == 1) {
							if (!strcmp(((char **) (theitem->addr))[0], "BIG_ENDIAN")) {
								if (NATIVE_CAT_BYTE_ORDER == LITTLE_ENDIAN_CAT_BYTE_ORDER) {
									swapbytesoninput = 1;
								}
							} else {
								if (!strcmp(((char **) (theitem->addr))[0], "LITTLE_ENDIAN")) {
									if (NATIVE_CAT_BYTE_ORDER == BIG_ENDIAN_CAT_BYTE_ORDER) {
										swapbytesoninput = 1;
								}
								} else {
									error_exit("illegal byteorder header value\n");
								}
							}
						}
					}
...
{
	item	*theitem, *byteorderheaderitem;
	comment	*thecomment;
	int	filetype;
	char	*byteorderstring;
	...
	/* create and install the byteorder header item */
	byteorderheaderitem = sscannewitem(TEXT_TYPE, "1 1 byteorder");
	allocitemcontents(byteorderheaderitem, &(byteorderheaderitem->addr), 0);
	byteorderstring = ((char **) (byteorderheaderitem->addr))[0] = (char *) calloc(64, sizeof(char));
	if (NATIVE_CAT_BYTE_ORDER == BIG_ENDIAN_CAT_BYTE_ORDER) {
		sprintf(byteorderstring, "BIG_ENDIAN");
	} else {
		sprintf(byteorderstring, "LITTLE_ENDIAN");
	}
	installitem(byteorderheaderitem, &(thecathead->headeritembase));
...
void		setheaderitemaddress(char *name, cathead *thecathead, void *theaddr)
{
	...
	if (theitem) {
		...
		fprintf(stderr, "setheaderitemaddress: no item named %s\n", name);
		exit(-1);
	}
}
