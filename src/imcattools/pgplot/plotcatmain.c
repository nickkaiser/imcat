...
int		main(int argc, char *argv[])	
{
	...
	float		ee, rr, rdefault, fillshade, theshade, aspectratio, width;
	...
	while (arg < argc) {
			...
			case 'K':
				...
				sscanf(argv[arg++], "%f", &(newlabel->y));
				newlabel->text = argv[arg++];
				sscanf(argv[arg++], "%d", &(newlabel->colorindex));
				newlabel->next = baselabel;
				baselabel = newlabel;
				break;
			case 'W':
	...
	/* now add the labels */
	while (baselabel) {
		if (baselabel->colorindex) {
			cpgqci(&oldci);
			cpgsci(baselabel->colorindex);
		}		
