...
main(int argc, char *argv[])
{
	...
	double	chi2;
	/* stuff for amoeba - v = vector of coefficients */
	double	**p, *y, ftol, *v;
	int	nfunk;
	...
	p = (double **) calloc(ncoefft + 1, sizeof(double *));
	for (i = 0; i < ncoefft + 1; i++) {
		p[i] = (double *) calloc(ncoefft, sizeof(double));
	}
	for (i = 0; i < ncoefft + 1; i++) {
		for (j = 0; j < ncoefft; j++) {
			p[i][j] = v[j] + 0.01 * (double) drand48();
		}
	}
	y = (double *) calloc(ncoefft + 1, sizeof(double));
	for (i = 0; i < ncoefft + 1; i++) {
		y[i] = chisquared(p[i]);
	}
	amoeba(p, y, ncoefft, ftol, chisquared, &nfunk);
