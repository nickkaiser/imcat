...
DESCRIPTION\n\
	...
	galaxies.  It assumes that galaxies are drawn from a Schechter\n\
	style luminosity function, are non-evolving, and are randomly placed\n\
	in comoving coordinates.  The cosmology is Einstein - de Sitter.\n\
\n\
...
\n\
	The csb is just (1 + z)^-4.\n\
	The angular size assumes that the luminosity scales\n\
	as the area and, for an L_star galaxy, is the angle\n\
	subtended by 3kpc in arc seconds.\n\
\n\
SEE ALSO\n\
	makemockimage\n\
\n\
...
main(int argc, char *argv[]){
	...
	while (1) {
		...
		m = ZEROPOINT + M + 5.0 * (log10(r * (1 + z)));
		/* compute redshift */
		z = pow(1.0 - r / (2 * HINV), -2) - 1;
		/* compute angular size */
		theta = 1.e-3 * LSTARSIZEINKPC * pow(10.0, -0.2 * (M - Mstar)) * (1 + z) / r;
		/* convert to arcseconds */
		theta *= 3600.0 * 180.0 / M_PI;
