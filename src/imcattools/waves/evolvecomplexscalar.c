...
/* we want this to be either 32 or 64 bit precision - defined by FPM = Floating Point Number = float/double */


static	fitsheader	*fits;

void    allocComplexArray(FPN complex ***f, int N1, int N2);
...
int	main (int argc, char* argv[]) 
{
	...
	FPN		**dreal, **dimag, **vreal, **vimag;
	FPN complex	**d, **v, laplacian, Ddx, Ddy;
	FPN		**Ax, **Ay, **Ftx, **Fty, **Fxy, **jx, **jy;
	FILE		*opstream[NOP];
	...
	allocRealArray(&dreal, Nx, Ny);
	allocRealArray(&vreal, Nx, Ny);
	allocRealArray(&dimag, Nx, Ny);
	allocRealArray(&vimag, Nx, Ny);

	/* and for the complex arrays */
	allocComplexArray(&d, Nx, Ny);
	allocComplexArray(&v, Nx, Ny);

	/* and for the EM field */
	allocRealArray(&Ax, Nx, Ny);
	allocRealArray(&Ay, Nx, Ny);
	allocRealArray(&Ftx, Nx, Ny);
	allocRealArray(&Fty, Nx, Ny);
	allocRealArray(&Fxy, Nx, Ny);

	/* and for the current */
	allocRealArray(&jx, Nx, Ny);
	allocRealArray(&jy, Nx, Ny);
	...
	for (iframe = 0; iframe < nframes; iframe++) {
		...
		for (istep = 0; istep < nsteps; istep++) {
			...
			for (y = 0; y < Ny; y++) {
				...
				for (x = 0; x < Nx; x++) {
					...
					jx[y][x] = -0.5 * q * CIMAG(d[y][x] * CONJ(Ddx) - CONJ(d[y][x]) * Ddx);
					jy[y][x] = -0.5 * q * CIMAG(d[y][x] * CONJ(Ddy) - CONJ(d[y][x]) * Ddy);
				}
			}								
			/* advance v, Ftx, Ftx */
			for (y = 0; y < Ny; y++) {
		...
		for (y = 0; y < Ny; y++) {
			for (x = 0; x < Nx; x++) {
				dreal[y][x] = CREAL(d[y][x]);
				dimag[y][x] = CIMAG(d[y][x]);
			}
		}
		...
		for (y = 0; y < Ny; y++) {
			...
			for (x = 0; x < Nx; x++) {
				xm = (Nx + x - 1) % Nx;
				xp = (x + 1) % Nx;
				dreal[y][x] = 0.5 * q * CIMAG(d[y][x] * CONJ(v[y][x]) - CONJ(d[y][x]) * v[y][x]);
...
void    allocComplexArray(FPN complex ***f, int N1, int N2)
{
        ...
        (*f) = (FPN complex **) calloc(N2, sizeof(FPN complex *));
        if (!*f)
                error_exit("allocComplexArray: memory allocation failure\n");
        (*f)[0] = (FPN complex *) calloc(N1 * N2, sizeof(FPN complex));
        if (!(*f)[0])
                error_exit("allocComplexArray: memory allocation failure\n");
        for (i = 1; i < N2; i++)
                (*f)[i] = (*f)[i - 1] + N1;
