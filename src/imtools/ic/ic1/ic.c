			...
			for (iop = 0; iop < nop; iop++) {
				...
				switch (theop->type) {
					case IM_VALUE_TYPE:
						...
						push((double) f[theop->opno][ix]);
						break;
					case CONSTANT_TYPE:
						push(theop->data);
						break;
					case NUM0_FUNC_TYPE:
						push(num0funcarray[theop->opno]());
						break;
					case NUM1_FUNC_TYPE:
						push(num1funcarray[theop->opno](pop()));
						break;
					case NUM2_FUNC_TYPE:
						op2 = pop();
						push(num2funcarray[theop->opno](pop(), op2));
						break;
					default:
						error_exit("ic: bad op type\n");
						break;				
				}
				if (!ok)
					break;
