...
void CDS_READCURSOR(int *sample, float *x, float *y, char *key, int *ier);
...
void CDS_COMPUTEZSCALE(uchar *pix, int *nx, int *ny, int *bitpix, float *z1, float *z2);
...
void CDS_READSUBRASTER(int *lx, int *ly, int *nx, int *ny, uchar *pix, int *ier);
void CDS_WRITESUBRASTER(int *lx, int *ly, int *nx, int *ny, uchar *pix, int *ier);
...
void    CDS_READIMAGE(), CDS_READFRAMEBUFFER(), CDS_READSUBRASTER();
void    CDS_WRITESUBRASTER(), CDS_SETCURSOR(), CDS_READCURSOR();
void    CDS_SELECTFB(), CDS_CLOSE(), CDS_COMPUTEZSCALE(), CDS_ZSCALEIMAGE();
...
void    CDS_SETLINESTYLE(), CDS_MARKCOORDSFILE();
...
void
CDS_READCURSOR (sample, x, y, key, ier)
int     *sample;                 	/* wait for keystroke?  */
...
{
	char	ch[2];
	CDF_READCURSOR (sample, x, y, ch, ier);
	strupk (ch, key, 2);
...
/*  CDS_COMPUTEZSCALE -- Compute the optimal z1/z2 values for an array.  */
...
void
CDS_COMPUTEZSCALE (pix, nx, ny, bitpix, z1, z2)
uchar   *pix;                   	/* data to be sampled   */
int     *nx, *ny;                 	/* image dimensions     */
int     *bitpix;                 	/* bits per pixel       */
...
/*  CDS_READSUBRASTER -- Read a rectangular region of the frame buffer.  */
...
void
CDS_READSUBRASTER (lx, ly, nx, ny, pix, ier)
...
/*  CDS_WRITESUBRASTER -- Write a rectangular region of the frame buffer.  */
...
void
CDS_WRITESUBRASTER (lx, ly, nx, ny, pix, ier)
int     *lx, *ly;              		/* region corner        */
int     *nx, *ny;                 	/* dimensions           */
uchar   *pix;                   	/* subraster pixels     */
int	*ier;				/* error code		*/
