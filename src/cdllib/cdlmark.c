...
/* NK - made this global and declared it in cdl.h
...
static MarkerPtr cdl_initEllAnnMarker();
/* NK made this global*/
MarkerPtr cdl_findNearest();
...
#define FONT_SCALE      0.4	/* default text font scale		*/
...
#endif
{
	...
	if (angle == 0.0) {
	    /* Optimize a bit for horizontal text. */
            ...
            ny = FONT_HEIGHT * (size * FONT_SCALE) + 10;
	    if (strstr(str, "\\fU")) {
		ny += ch - (ch * 0.666) / 2;
	    }
	    if (strstr(str, "\\fD")) {
	    ...
	    double 	coso, sino;
            ...
            coso = cos ((double)(-angle * (double)3.14159265358979323846/180.));
            sino = sin ((double)(-angle * (double)3.14159265358979323846/180.));

	    xc[0] = x;					 /* LL corner */
	    yc[0] = y;
	    xc[1] = x + ch * sino;			/* UL corner */
	    yc[1] = y - ch * coso;
	    xc[2] = x + len * coso + ch * sino;       	/* UR corner */
	    yc[2] = y - len * sino + ch * coso;
	    xc[3] = x + len * coso;    	       		/* LR corner */
	    yc[3] = y - len * sino;

	    if (strstr(str, "\fU")) {
		yc[1] = y + (ch + ch / 2) * coso;
	        yc[2] = y - len * sino + (ch + ch / 2) * coso;
	    } 
	    if (strstr(str, "\fD"))
		yc[0] = y - (ch + ch / 2);
...
/* NK made cdl_findNearest global by removing:
static MakerPtr
in favour of
MarkerPtr */
MarkerPtr 
...
#endif
{
	 ...
	 * necessarily so we cna maintain the spacing.
	 */
	if (pix == color)
	    return (pix);
