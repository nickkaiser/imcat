...
#endif
{
	...
	status =  imd_readCursor (cdl->imd, sample, x, y, key);
	return (*key);
}
...
#endif
{
        cdl_zscale ((uchar *)pix, nx, ny, bitpix, z1, z2, cdl->contrast,
	    cdl->nsample, (int)(cdl->nsample / cdl->nsamplines));
...
#endif
{
	...
	float 		pval, scale, dscale = (200.0 / 3.0);
	uchar	 	*outpix ;
	float	 	z1, z2;			/* zscale values 	*/
	int	 	zt;
	extern		void bcopy();
